...
type Page = "dashboard" | "projects" | "templates" | "projectsTracker" | "tasksTracker" | "timelineChart" | "mobilization" | "members" | "membersCalendar" | "dataUpload" | "settingsPMs" | "settingsMembers";
...
const PROJECTS_TRACKER_FIXED_TOP_HEIGHT = 81 + PROJECTS_TRACKER_TABLE_TOP_MAIN - NAV_TOP;
...
const MEMBERS_CALENDAR_UTIL_TO_TABLE_GAP =
  MEMBERS_CALENDAR_DATA_TABLE_TOP - (MEMBERS_CALENDAR_UTIL_TOP - 81) - MEMBERS_CALENDAR_UTIL_HEIGHT;
...
const MEMBERS_CALENDAR_FILTER_PANEL_HEIGHT =
  MEMBERS_CALENDAR_FILTER_PANEL_PADDING_Y * 2 + MEMBERS_CALENDAR_FILTER_PANEL_CONTENT_HEIGHT;
...
const MEMBERS_CALENDAR_UTIL_TOP_WITH_FILTERS =
  MEMBERS_CALENDAR_FILTER_PANEL_TOP + MEMBERS_CALENDAR_FILTER_PANEL_HEIGHT + MEMBERS_CALENDAR_FILTER_PANEL_MARGIN;
...
const MEMBERS_CALENDAR_FIXED_TOP_HEIGHT_WITH_FILTERS =
  MEMBERS_CALENDAR_UTIL_TOP_WITH_FILTERS + MEMBERS_CALENDAR_UTIL_HEIGHT + MEMBERS_CALENDAR_UTIL_BOTTOM_PADDING - NAV_TOP;
const MEMBERS_CALENDAR_FILTERS_LAYOUT_OFFSET =
  MEMBERS_CALENDAR_FIXED_TOP_HEIGHT_WITH_FILTERS - MEMBERS_CALENDAR_FIXED_TOP_HEIGHT;
...
function ScreenComponent({ page }: { page: Page }) {
  switch (page) {
    case "dashboard":       return <V201Dashboard />;
    case "projects":        return <V202Projects />;
    case "templates":       return <V203Templates />;
    case "projectsTracker": return <V204ProjectsTracker />;
    case "tasksTracker":    return <V205TaskTracker />;
    case "timelineChart":   return <V206TimelineChart />;
    case "mobilization":    return <V207Mobilization />;
    case "members":         return <V208Members />;
    case "membersCalendar": return <V209MembersCalendar />;
    case "dataUpload":      return <V210DataUpload />;
    case "settingsPMs":     return <V21101SettingsPMs />;
    case "settingsMembers": return <V21102SettingsMembers />;
  }
}
...
const MEMBERS_FILTER_PANEL_TOP =
  MEMBERS_SEARCH_TOP + MEMBERS_SEARCH_HEIGHT + MEMBERS_CALENDAR_FILTER_PANEL_MARGIN;
...
const MEMBERS_FILTERS_LAYOUT_OFFSET =
  MEMBERS_CONTENT_TOP_WITH_FILTERS - (MEMBERS_MAIN_TOP + MEMBERS_FIRST_CARD_TOP);
      ...
      <div style={{ position: "relative" }}>
        ...
        <input
          ...
          style={{
            ...
            lineHeight: "20px",
            outline: "none",
            boxShadow: open ? "0 0 0 3px rgba(67, 141, 253, 0.12)" : "none",
        ...
        >
          <div style={{ padding: "6px 0 8px" }}>
            {groups.length > 0 ? (
                ...
                <div key={group}>
                  ...
                  {items.map((item) => (
                    <button
                      ...
                      onMouseEnter={(event) => {
                        event.currentTarget.style.background = "#f5fbff";
                      }}
                      onMouseLeave={(event) => {
              ...
              <div style={{ padding: "12px 14px", fontSize: 13, color: "#737373" }}>
                No results found
              </div>
            )}
          </div>
...
const PROJECT_FILTERS = [
  ...
  { id: "pms", label: "All PMs", options: ["All PMs", "Alex Morgan", "Julie Kim"] },
  { id: "types", label: "All Types", options: ["All Types", "UX/UI Design", "Brand Strategy"] },
  { id: "clients", label: "All Clients", options: ["All Clients", "Northstar Inc.", "Atlas Co."] },
  {
    id: "contract",
...
function FilterDropdown({
  ...
  return (
    <div ref={containerRef} style={{ position: "relative", lineHeight: 0 }}>
      ...
      >
        {leading}
        <span style={{ overflow: "hidden", textOverflow: "ellipsis", minWidth: 0 }}>{value}</span>
        <span
          style={{
            marginLeft: "auto",
            flexShrink: 0,
          ...
          {options.map((option) => (
            <button
              ...
              style={{
                ...
                padding: "7px 14px",
                border: "none",
                background: option === value ? "#f5fbff" : "transparent",
                cursor: "pointer",
                fontSize: 13,
                lineHeight: "20px",
                color: "#17171a",
              ...
              onMouseLeave={(event) => {
                event.currentTarget.style.background =
                  option === value ? "#f5fbff" : "transparent";
              }}
            >
              {option}
            </button>
...
function StatusChevron({ color = "#3170AD" }: { color?: string }) {
  return (
    <svg width="12" height="12" viewBox="0 0 12 12" fill="none" aria-hidden style={{ flexShrink: 0, marginLeft: "auto" }}>
      <path d="M3 4.5L6 7.5L9 4.5" stroke={color} strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function ProjectsFilterBar() {
  const [openFilterId, setOpenFilterId] = useState<ProjectFilterId | null>(null);
  const [values, setValues] = useState<Record<ProjectFilterId, string>>(() =>
    Object.fromEntries(PROJECT_FILTERS.map((filter) => [filter.id, filter.label])) as Record<
      ProjectFilterId,
      string
    >,
  );
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8, pointerEvents: "auto" }}>
        {PROJECT_FILTERS.map((filter) => (
          <FilterDropdown
            ...
            onSelect={(value) => setValues((current) => ({ ...current, [filter.id]: value }))}
          />
        ))}
      </div>
      ...
      <button
        type="button"
        style={{ ...filterButtonStyle, marginLeft: "auto", pointerEvents: "auto" }}
      >
        Show Closed
      </button>
      ...
      >
        ...
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden>
            <rect x="2" y="2" width="5" height="5" rx="1" stroke="#3170AD" strokeWidth="1.33" />
            <rect x="9" y="2" width="5" height="5" rx="1" stroke="#3170AD" strokeWidth="1.33" />
            <rect x="2" y="9" width="5" height="5" rx="1" stroke="#3170AD" strokeWidth="1.33" />
            <rect x="9" y="9" width="5" height="5" rx="1" stroke="#3170AD" strokeWidth="1.33" />
          </svg>
        </div>
        <div
          style={{
        ...
        >
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden>
            <path d="M2 4H2.00667" stroke="#B4B4B4" strokeWidth="1.33" strokeLinecap="round" />
            <path d="M2 8H2.00667" stroke="#B4B4B4" strokeWidth="1.33" strokeLinecap="round" />
            <path d="M2 12H2.00667" stroke="#B4B4B4" strokeWidth="1.33" strokeLinecap="round" />
            <path d="M5.333 4H14" stroke="#B4B4B4" strokeWidth="1.33" strokeLinecap="round" />
            <path d="M5.333 8H14" stroke="#B4B4B4" strokeWidth="1.33" strokeLinecap="round" />
            <path d="M5.333 12H14" stroke="#B4B4B4" strokeWidth="1.33" strokeLinecap="round" />
          </svg>
        </div>
      </div>
    </div>
...
const TRACKER_PROJECT_DROPDOWN_OFFSET_LEFT =
  TRACKER_EDIT_BTN_OFFSET_LEFT + TRACKER_EDIT_BTN_WIDTH + TRACKER_HEADER_BTN_GAP;

const HEADER_PROJECT_DROPDOWN_LEFT =
  TRACKER_MAIN_LEFT + TRACKER_HEADER_INSET_LEFT + TRACKER_PROJECT_DROPDOWN_OFFSET_LEFT;
...
const TRACKER_FILTERS = [
  { id: "phases", label: "All Phases", options: ["All Phases", "Starting", "MVP", "Launch"] },
  { id: "types", label: "All Types", options: ["All Types", "UX/UI Design", "Brand Strategy"] },
  {
    id: "statuses",
    label: "All Statuses",
...
function ProjectsTrackerDropdowns() {
  ...
  const [filterValues, setFilterValues] = useState<Record<TrackerFilterId, string>>(() =>
    Object.fromEntries(TRACKER_FILTERS.map((filter) => [filter.id, filter.label])) as Record<
      TrackerFilterId,
      string
    >,
  );
      ...
      >
        {TRACKER_FILTERS.map((filter) => (
          <FilterDropdown
            ...
            onSelect={(value) => setFilterValues((current) => ({ ...current, [filter.id]: value }))}
            leading={filter.id === "phases" ? <TrackerFilterIcon /> : undefined}
            buttonStyle={filter.id === "phases" ? { gap: 10, paddingLeft: 10 } : undefined}
          />
        ))}
      </div>
    </div>
...
function ProjectsTrackerViewTasksLink({ onNavigate }: { onNavigate: () => void }) {
  return (
    <button
      type="button"
...
const TRACKER_TABLE_STATUS_OPTIONS = ["Not Started", "Started", "Complete"] as const;
...
const PROJECTS_TRACKER_STATUS_ROWS = [
  ...
  { id: "pt-project-1", rowTop: 299, initial: "Started", statusTopOffset: 13 },
] as const;

type ProjectsTrackerStatusRowId = (typeof PROJECTS_TRACKER_STATUS_ROWS)[number]["id"];

function ProjectsTrackerStatusDropdowns() {
  const [openId, setOpenId] = useState<ProjectsTrackerStatusRowId | null>(null);
  const [values, setValues] = useState<Record<ProjectsTrackerStatusRowId, string>>(() =>
    Object.fromEntries(
      PROJECTS_TRACKER_STATUS_ROWS.map((row) => [row.id, row.initial]),
        ...
        <div
          ...
          style={{
            ...
            top:
              ...
              row.rowTop +
              ("statusTopOffset" in row ? row.statusTopOffset : TRACKER_TABLE_STATUS_TOP_OFFSET),
            pointerEvents: "auto",
          }}
        >
          <TableStatusDropdown
            ...
            onSelect={(value) => setValues((current) => ({ ...current, [row.id]: value }))}
...
const TASKS_FILTERS = [
  ...
  },
  {
    id: "assignees",
    label: "All Assignees",
    options: ["All Assignees", "Alex Morgan", "Jesse Li", "Sarah Chen", "Unassigned"],
  },
] as const;
...
function TasksTrackerDropdowns() {
  ...
  const [project, setProject] = useState<string>(TRACKER_PROJECTS[0]);
  const [filterValues, setFilterValues] = useState<Record<TasksFilterId, string>>(() =>
    Object.fromEntries(TASKS_FILTERS.map((filter) => [filter.id, filter.label])) as Record<
      TasksFilterId,
      string
    >,
  );
      ...
      >
        {TASKS_FILTERS.map((filter) => (
          <FilterDropdown
            ...
            onOpenChange={(open) => setOpenId(open ? filter.id : null)}
            onSelect={(value) => setFilterValues((current) => ({ ...current, [filter.id]: value }))}
            leading={filter.id === "statuses" ? <TrackerFilterIcon /> : undefined}
            buttonStyle={filter.id === "statuses" ? { gap: 10, paddingLeft: 10 } : undefined}
          />
        ))}
      </div>
    </div>
...
function TasksTrackerViewProjectsLink({ onNavigate }: { onNavigate: () => void }) {
  return (
    <button
      type="button"
...
const TASKS_TABLE_STATUS_OPTIONS = ["Not Started", "In Progress", "Complete"] as const;
...
function TasksTrackerStatusDropdowns() {
  ...
  const [values, setValues] = useState<Record<TasksTrackerStatusRowId, string>>(() =>
    Object.fromEntries(
...
const TEMPLATE_TABS: readonly { id: TemplatesTabId; label: string; left: number }[] = [
...
const CALENDAR_ROLE_OPTIONS = ["All Roles", "Designer", "Developer", "PM"] as const;
const CALENDAR_PROJECT_OPTIONS = ["All Projects", "Northstar Rebrand", "Atlas Mobile App"] as const;
const CALENDAR_SORT_OPTIONS = ["Name A–Z", "Name Z–A", "Utilization High–Low", "Utilization Low–High"] as const;
...
const RESOURCES_FILTER_FIELD_BLOCK_HEIGHT =
  RESOURCES_FILTER_LABEL_HEIGHT + RESOURCES_FILTER_FIELD_GAP + CALENDAR_FILTER_CONTROL_HEIGHT;
...
function CalendarIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden>
      ...
      <path d="M2 2.66667H14C14.7364 2.66667 15.3333 3.26362 15.3333 4V13.3333C15.3333 14.0697 14.7364 14.6667 14 14.6667H2C1.26362 14.6667 0.666667 14.0697 0.666667 13.3333V4C0.666667 3.26362 1.26362 2.66667 2 2.66667Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
...
function PlusIcon({ color = "#F5FBFF" }: { color?: string }) {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden>
      <path d="M3.33333 8H12.6667" stroke={color} strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
      <path d="M8 3.33333V12.6667" stroke={color} strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
    </svg>
  );
}
...
function ResourcesFilterBar({
  ...
  const [openId, setOpenId] = useState<"role" | "project" | "sort" | null>(null);
  ...
  return (
    <div style={resourcesFilterBarOuterStyle}>
      ...
      {showSearch && (
        <div style={{ ...resourcesFilterFieldColumnStyle, flex: "1 1 280px", minWidth: 180 }}>
          <div style={calendarFilterFieldLabelStyle}>Search by name / role</div>
          <SearchField
            containerStyle={{ position: "relative", width: "100%", lineHeight: 0 }}
            width={280}
            height={CALENDAR_FILTER_CONTROL_HEIGHT}
            placeholder="Search..."
    ...
    >
      ...
      >
        ...
        >
          {CALENDAR_VIEW_MODES.map((mode) => {
            ...
            return (
              ...
              >
                {mode.id === "daily" && isActive && <CalendarIcon />}
                {mode.label}
              </button>
            );
          })}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
          ...
          >
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden>
              <path d="M10 12L6 8L10 4" stroke="#3170AD" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
            </svg>
          </button>
          <span
            style={{
          ...
          >
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden>
              <path d="M6 12L10 8L6 4" stroke="#3170AD" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.33333" />
            </svg>
          </button>
        </div>
      ...
      <div
        style={{
          position: "absolute",
          left: MEMBERS_CALENDAR_CONTENT_LEFT,
          top: filtersActive ? MEMBERS_CALENDAR_UTIL_TOP_WITH_FILTERS : MEMBERS_CALENDAR_UTIL_TOP,
          width: MEMBERS_CALENDAR_CONTENT_WIDTH,
          height: 24,
          display: "flex",
          alignItems: "center",
...
export default function App() {
  ...
  const [membersCalendarFiltersActive, setMembersCalendarFiltersActive] = useState(false);
  ...
  const handleNavClick = (e: React.MouseEvent) => {
    ...
    if (target.closest("[data-name='Dashboard Tab']"))         { setPage("dashboard"); return; }
    if (target.closest("[data-name='Logo & Name Container']")) { setPage("dashboard"); return; }
    if (target.closest("[data-name='Projects']"))         { setPage("projects"); return; }
    if (target.closest("[data-name='Active Card 1']"))    { setPage("projects"); return; }
    if (target.closest("[data-name='Completed Card']"))   { setPage("projects"); return; }
    if (target.closest("[data-name='Active Card 2']"))    { setPage("members"); return; }
    if (target.closest("[data-name='Warning Card']"))     { setPage("membersCalendar"); return; }
    if (target.closest("[data-name='Dashboard All link']"))      { setPage("projects"); return; }
    if (target.closest("[data-name='Dashboard Projects link']")) { setPage("projects"); return; }
    if (target.closest("[data-name='Dashboard Calendar link']")) { setPage("membersCalendar"); return; }
    if (target.closest("[data-name='New Project btn']")) { setNewProjectOpen(true); return; }
    if (target.closest("[data-name='Templates']"))        { setPage("templates"); return; }
    if (target.closest("[data-name='Templates btn']"))    { setPage("templates"); return; }
    if (target.closest("[data-name='Projects Tracker']")) { setPage("projectsTracker"); return; }
    if (target.closest("[data-name='View Tasks']")) {
      setPage(page === "tasksTracker" ? "projectsTracker" : "tasksTracker");
      return;
    }
    if (target.closest("[data-name='Tasks Tracker']"))    { setPage("tasksTracker"); return; }
    if (target.closest("[data-name='Timeline Chart']"))       { setPage("timelineChart"); return; }
    if (target.closest("[data-name='Project Mobilization']")) { setPage("mobilization"); return; }
    if (target.closest("[data-name='Project Members']"))      { setPage("members"); return; }
    if (target.closest("[data-name='Members Calendar']"))     { setPage("membersCalendar"); return; }
    if (target.closest("[data-name='Data Upload']"))          { setPage("dataUpload"); return; }
    if (target.closest("[data-name='Configuration']"))        { setPage("settingsPMs"); return; }
    if (target.closest("[data-name='Tab-Members']"))          { setPage("settingsMembers"); return; }
    if (target.closest("[data-name='Tab-PMs']"))              { setPage("settingsPMs"); return; }
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#f5fbff", overflow: "hidden" }}>
      ...
      <div
        ...
        style={{
          position: "fixed",
          left: 0, top: 0, width: NAV_LEFT, bottom: 0,
          overflow: "hidden",
          zIndex: 10,
        }}
      >
        <div style={{ position: "absolute", top: 0, left: 0, width: SCREEN_W, height: SCREEN_H }}>
      ...
      <div
        ...
        style={{
          position: "fixed",
          left: NAV_LEFT, top: 0, right: 0, height: NAV_TOP,
          overflow: "hidden",
          zIndex: 10,
        }}
      >
        <div style={{ position: "absolute", top: 0, left: -NAV_LEFT, width: SCREEN_W, height: SCREEN_H }}>
          <ScreenComponent page={page} />
        </div>
      </div>
      ...
      {page === "projectsTracker" && (
        ...
        >
          ...
          >
            ...
            <ProjectsTrackerViewTasksLink onNavigate={() => setPage("tasksTracker")} />
          </div>
        </div>
      )}
        ...
        >
          ...
          >
            ...
            <MembersCalendarControls
              ...
              onFiltersActiveChange={setMembersCalendarFiltersActive}
            />
          </div>
        </div>
      )}
      ...
      <div style={{ position: "fixed", left: 0, top: NAV_TOP, right: 0, height: 1, background: "#d9d9d9", zIndex: 15, pointerEvents: "none" }} />
      <div style={{ position: "fixed", left: NAV_LEFT, top: 0, width: 1, bottom: 0, background: "#d9d9d9", zIndex: 15, pointerEvents: "none" }} />

      {/* Main content area — scrollable, component shifted so main aligns to scroll origin */}
      <div
        data-scroll-content
        data-members-calendar-filters-active={
          page === "membersCalendar" && membersCalendarFiltersActive ? "" : undefined
        }
        data-members-filters-active={page === "members" && membersFiltersActive ? "" : undefined}
        onClick={handleNavClick}
        style={{
          position: "fixed",
          left: NAV_LEFT, top: NAV_TOP, right: 0, bottom: 0,
          overflow: "auto",
          zIndex: 5,
      ...
      >
        ...
        >
          <div style={{ position: "absolute", top: -NAV_TOP, left: -NAV_LEFT, width: SCREEN_W, height: SCREEN_H }}>
            <ScreenComponent page={page} />
            {page === "projects" && (
              <>
                <ProjectsSearch />
            ...
            {(page === "settingsPMs" || page === "settingsMembers") && (
              <>
                <div
                  onClick={() => setPage("settingsPMs")}
                  style={{ position: "absolute", left: 297, top: 203, width: 545, height: 46, cursor: "pointer", zIndex: 20 }}
                />
                <div
                  onClick={() => setPage("settingsMembers")}
                  style={{ position: "absolute", left: 854, top: 203, width: 545, height: 46, cursor: "pointer", zIndex: 20 }}
                />
              </>
            )}
            {page === "tasksTracker" && (
              <>
                ...
                <TasksTrackerViewProjectsLink onNavigate={() => setPage("projectsTracker")} />
                <TasksTrackerStatusDropdowns />
              </>
            )}
            {page === "projectsTracker" && <ProjectsTrackerStatusDropdowns />}
